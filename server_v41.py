...
# Remove the old base-page duplicate "try another scenario" button; keep the one inside the payment panel.
html=html.replace('<button class="btn ghost" style="width:100%;margin-top:14px" onclick="chosen=null;sid=null;renderDomains();go(\'choose\')" data-t="again">Try another scenario</button>','')
# Force readable payment-card content on mobile and add progress-state visuals.
html=html.replace('</head>',r'''<style id="mamV422fix">
...
# Drive the four indicators from empty -> active spinner -> check, spread across the real analysis wait.
html=html.replace('</body>',r'''<script id="mamProgress422">
(function(){
 ...
 const observer=new MutationObserver(()=>{
   if(ov.classList.contains('on')){
     clearTimers();reset();working(0);
     timers.push(setTimeout(()=>{done(0);working(1)},900));
     timers.push(setTimeout(()=>{done(1);working(2)},2300));
     timers.push(setTimeout(()=>{done(2);working(3)},4200));
   }else{
     clearTimers();
   }
 });
...
if __name__=='__main__':
    os.chdir(app.ROOT)
    print('Move A Mind v4.22 - progressive analysis checks + readable payment cards')
