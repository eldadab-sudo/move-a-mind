...
if start>=0 and end>start:
 pay=r'''function renderLocked(j){const p=j.preview||j,he=lang==='he';$('#outcome').textContent='';$('#freeInsight').innerHTML=`<div class="insight"><h3>${he?'טעימה מהניתוח שלך':'A glimpse of your analysis'}</h3><p>${p.top_insight||''}</p></div>`;$('#fullReport').innerHTML=`<div class="mamLockedPanel"><div class="mamLock">🔒</div><h2>${he?'המשך הניתוח נעול':'The rest of the analysis is locked'}</h2><p>${he?'כדי לצפות בכל התובנות, ההסברים וההמלצות המעשיות — בחר מסלול שמתאים לך.':'Choose a plan to see all insights, explanations and practical recommendations.'}</p></div>`;const box=$('#paywall');box.style.display='block';box.innerHTML=`<div class="premium" dir="${he?'rtl':'ltr'}"><div style="display:flex;align-items:center;gap:10px;margin-bottom:10px"><img src="/move-a-mind-logo.webp" style="width:58px;height:58px;border-radius:14px" alt="Move A Mind"><b>MOVE A MIND</b></div><h2 style="font-size:34px;line-height:1.15;margin:10px 0">${he?'רוצה לדעת מה באמת קרה בשיחה שלך?':'Want to know what really happened in your conversation?'}</h2><p style="font-size:17px;line-height:1.55">${he?'קיבלת עכשיו טעימה מהניתוח. המשך הדוח מחכה לך — עם כל התובנות, ההסברים וההמלצות המעשיות.':'You just received a preview. Your full report is ready with all insights, explanations and practical recommendations.'}</p><input id="mamEmail" class="checkoutEmail" type="email" placeholder="Email" autocomplete="email"><div class="mamPayGrid"><div class="mamPlanCard"><div class="mamPlanName">Deep Report</div><div class="mamPrice">$4.90</div><div class="mamPeriod">${he?'חד־פעמי':'one-time'}</div><ul class="mamBenefits"><li>${he?'דוח מלא חד־פעמי':'One full report'}</li><li>${he?'כל התובנות וההמלצות':'All insights and recommendations'}</li><li>${he?'גישה מיידית':'Immediate access'}</li><li>${he?'ללא התחייבות':'No commitment'}</li></ul><button class="mamPlanBtn" data-plan="deep">${he?'בחר מסלול':'Choose plan'}</button></div><div class="mamPlanCard"><div class="mamPlanName">Pro Monthly</div><div class="mamPrice">$15</div><div class="mamPeriod">/month</div><ul class="mamBenefits"><li>${he?'גישה מלאה לחודש':'Full access for a month'}</li><li>${he?'כל הדוחות והכלים':'All reports and tools'}</li><li>${he?'עדכונים ושיפורים':'Updates and improvements'}</li><li>${he?'ביטול בכל עת':'Cancel anytime'}</li></ul><button class="mamPlanBtn" data-plan="monthly">${he?'בחר מסלול':'Choose plan'}</button></div><div class="mamPlanCard best"><div class="mamBadge">☆ ${he?'הכי משתלם':'Best value'}</div><div class="mamPlanName">Pro Annual</div><div class="mamPrice">$129</div><div class="mamPeriod">/year</div><ul class="mamBenefits"><li>${he?'גישה מלאה לשנה':'Full access for a year'}</li><li>${he?'כל הדוחות והכלים':'All reports and tools'}</li><li>${he?'עדכונים ושיפורים':'Updates and improvements'}</li><li>${he?'חיסכון של כ־30%':'Save about 30%'}</li></ul><button class="mamPlanBtn" data-plan="annual">${he?'בחר מסלול':'Choose plan'}</button></div></div><div class="mamFeatureRow"><div><div class="mamFeatureIcon">🎯</div>${he?'המלצות מעשיות לשיחה הבאה':'Practical next-step advice'}</div><div><div class="mamFeatureIcon">💡</div>${he?'איפה אפשר להשתפר':'Where to improve'}</div><div><div class="mamFeatureIcon">▥</div>${he?'מה עבד טוב בשיחה שלך':'What worked well'}</div></div><div class="micro">${he?'תשלום מאובטח באמצעות Stripe':'Secure checkout with Stripe'}</div><button class="mamRetry" onclick="chosen=null;sid=null;renderDomains();go('choose')">↻ ${he?'נסה תרחיש נוסף':'Try another scenario'}</button></div>`;box.querySelectorAll('.mamPlanBtn').forEach(b=>b.onclick=()=>checkoutPlan(b.dataset.plan,b))}
...
if __name__=='__main__':
 os.chdir(app.ROOT);print('Move A Mind v4.20 approved loader + payment wall');ThreadingHTTPServer(('0.0.0.0',app.PORT),H).serve_forever()
